...
struct ChannelFactory {
    ...
    static func generateChannels(_ number: Int, withShows showsNumber: Int) -> [Channel] {
        
        var channels = [Channel]()
        for i in 0 ..< number {
            let events = ChannelFactory.generateEvents(showsNumber, inRow: i)
            var channel = Channel()
            channel.events = events
            channel.channelImage = ChannelFactory.channelsImages[i]
            channels.append(channel)
        }
        return channels
    }
    
    static func generateEvents(_ number: Int, inRow rowIndex: Int) -> [Event] {
        var events = [Event]()
        var date = Date.add(days: -1)
        for _ in 0 ..< number {
            let randomEvent = ChannelFactory.randomShows[rowIndex][Int(arc4random_uniform(6))]
